...
__interrupt void sw2(void)
{
	 ...
	 switch(P2IV)
	 {
	  ...
	  if((P2IN & 0x40) == 0)
	  {
		  if(mode==2) msec=0,sec=0;
		  else if(mode==1) lsec=sec,lmsec=msec;
	  }
...
__interrupt void timer0_handler(void)
{
	if(mode==1)
	{
	 ...
	 if(mode==2)__no_operation();
	 if((P2IN & 0x40)==0) longsw1++;
}
		...
		for(i=0;i<3;i++)
		{
			...
			if(rd != 0x0f)
			{
				if(flag==1)
				{
					flag=0;
					if(rd==0x0e) b=1+i;
					else if(rd==0x0d) b=4+i;
					else if(rd==0x0b) b=7+i;
					else if(rd==0x07)b=10+i;
				}
			}
			else flag=1;
		}
